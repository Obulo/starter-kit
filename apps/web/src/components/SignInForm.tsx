...
export function SignInForm() {
  ...
  return (
    <div className="w-full bg-card text-card-foreground rounded-lg border shadow-sm p-6 space-y-6">
      {/* Header */}
      <div className="flex flex-col space-y-2 text-center">
        <h1 className="text-2xl font-semibold tracking-tight">
          {step === 'email' ? (
            orgConfig.hasCustomBranding() ? 'Welcome Back' : 'Welcome back'
          ) : 'Enter your password'}
        </h1>
        <p className="text-sm text-muted-foreground">
          {step === 'email' 
            ? (orgConfig.hasCustomBranding() 
                ? `Enter your email to access ${orgConfig.name}`
